...
ui <- page_sidebar(
  ...
  sidebar = sidebar(
    ...
    div(
      ...
      div(style = "font-size: 12px; opacity: 0.75; text-align: center; margin-top: 10px; line-height: 1.4; padding: 0 4px;",
          textOutput("pulse_description", inline = TRUE)),
      div(style = "font-size: 10px; opacity: 0.4; text-align: center; margin-top: 6px;",
...
server <- function(input, output, session) {
  ...
  # Pulse uses the full dataset, not the filtered view — stable reading of Earth
  pulse <- reactive({
  ...
  output$pulse_description <- renderText({
    pulse_description(pulse()$pulse)
  })
